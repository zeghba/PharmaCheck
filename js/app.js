...
(function () {
  ...
  /* ================================================================== *
   * 9. Connect — which pharmacy this phone belongs to
   *
   * Deliberately two fields. Creating a pharmacy, adding accounts and
   * configuring the database all live in the separate admin app, which
   * is the only thing that holds the credentials for any of it.
   * ================================================================== */
  ...
  function renderSettings() {
    ...
    $('#set-sub').textContent = c.mode === Cloud.OFF
      ...
      : c.pharmacy
        ? 'Pharmacy “' + c.pharmacy + '”'
        : 'Which pharmacy this phone belongs to';
  ...
  $('#set-test').addEventListener('click', function () {
    ...
    if (!url) { setResult('#set-result', 'Enter the server address first', 'bad'); return; }
    setResult('#set-result', 'Checking…', 'busy');
    Cloud.health(url).then(function (body) {
      setResult('#set-result', 'Server is up (v' + body.version + ')', 'ok');
    }, function (e) {
      setResult('#set-result', e.message, 'bad');
    });
  });

  /* Connecting is also the first sync: a phone that has been running on its
     own either seeds an empty pharmacy with what it holds, or takes the
  ...
  $('#set-save').addEventListener('click', function () {
    ...
    Cloud.listAccounts(pharmacy).then(function (accounts) {
      setResult('#set-result',
        ...
        '. Sign out and back in to start syncing.', 'ok');
      renderSettings();
    }, function (e) {
      setResult('#set-result', e.message, 'bad');
      paintSyncCard();
  ...
  $('#sync-now').addEventListener('click', function () {
    ...
    Cloud.sync().then(function (result) {
      if (result.skipped === 'signed-out') {
        ...
        toast(result.message || 'Sync failed');
      }
      renderSettings();
  ...
  /* Every mutation lands in the outbox; this drains it a moment later, so a
     sale syncs on its own rather than waiting for someone to open Connect.
     Failures are silent here — the Connect card is where they are reported,
     and a vendor mid-sale cannot act on them anyway. */
  var syncTimer = null;
